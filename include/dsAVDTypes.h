 ...
 * - YCbCr:   Y is luma (brightness), Cb is blue minus luma (B-Y) and Cr is red
 * minus luma (R-Y)
 ...
 * - SCART:   SCART stands for Syndicat des
 * Constructeursd’AppareilsRadiorécepteurs et Téléviseurs or Radio and
 ...
 * Described herein are the DeviceSettings HAL types and functions that are part
 * of the FPD subsystem. The FPD subsystem manages system-specific HAL
 * operations.
 *  @{
 */
...
typedef enum _dsVideoPortType_t {
  ...
  dsVIDEOPORT_TYPE_INTERNAL,   ///< Internal (integrated/internal display) video
                               ///< output
...
#define dsVideoPortType_isValid(t)                                             \
  (((t) >= dsVIDEOPORT_TYPE_RF) && ((t) < dsVIDEOPORT_TYPE_MAX))
...
#define dsAudioType_isValid(t)                                                 \
  (((t) >= dsAUDIOPORT_TYPE_ID_LR) && ((t) < dsAUDIOPORT_TYPE_MAX))
...
#define dsAudioEncoding_isValid(t)                                             \
  (((t) >= dsAUDIO_ENC_NONE) && ((t) < dsAUDIO_ENC_MAX))
...
#define dsAudioCompression_isValid(t)                                          \
  (((t) >= dsAUDIO_CMP_NONE) && ((t) < dsAUDIO_CMP_MAX))
...
#define dsAudioStereoMode_isValid(t)                                           \
  (((t) >= dsAUDIO_STEREO_UNKNOWN) && ((t) < dsAUDIO_STEREO_MAX))
...
typedef enum _dsAudioFormat_t {
  ...
  dsAUDIO_FORMAT_DOLBY_MAT,    ///< Metadata-enhanced Audio Transmission audio
                               ///< format
  dsAUDIO_FORMAT_DOLBY_TRUEHD, ///< Dolby TrueHD audio format
  dsAUDIO_FORMAT_DOLBY_EAC3_ATMOS,   ///< Audio Compression 3 Dolby Atmos audio
                                     ///< format
  dsAUDIO_FORMAT_DOLBY_TRUEHD_ATMOS, ///< Dolby TrueHD Dolby Atmos audio format
  dsAUDIO_FORMAT_DOLBY_MAT_ATMOS,    ///< Metadata-enhanced Audio Transmission
                                     ///< Dolby Atmos audio format
  dsAUDIO_FORMAT_DOLBY_AC4_ATMOS,    ///< Audio Compression 4 Dolby Atmos audio
                                     ///< format
...
typedef enum _dsMS12Capabilities_t {
  ...
  dsMS12SUPPORT_InteligentEqualizer =
      (1 << 1), ///< MS12 supported Intelligent Equalizer
  dsMS12SUPPORT_DialogueEnhancer =
      (1 << 2),                            ///< MS12 Dialogue Enhancer supported
...
typedef struct _dsMS12AudioProfileList_t {
  char audioProfileList
      [MAX_PROFILE_LIST_BUFFER_LEN]; ///< buffer containing the list of comma
                                     ///< separated audio profile names for menu
                                     ///< list (Supported MS12 Audio Profiles:
                                     ///< "Music,Movie,Sports,Entertainment,Night,Party,User")
...
typedef enum _dsATMOSCapability_t {
  ...
  dsAUDIO_ATMOS_DDPLUSSTREAM,  ///< can handle dd plus stream which is only way
                               ///< to pass ATMOS metadata
...
typedef struct _dsAudioTypeConfig_t {
  ...
  size_t numSupportedCompressions; ///< Number of supported audio compression
                                   ///< methods
  const dsAudioCompression_t
      *compressions;            ///< List of audio compression methods supported
...
/** @addtogroup DSHAL_VIDEOPORT_TYPES Device Settings HAL VideoPort Type
 * Definitions
 *  @ingroup DSHAL_VIDEOPORT
 *  @{
 */
...
#define dsVideoPortPixelResolution_isValid(t)                                  \
  (((t) >= dsVIDEO_PIXELRES_720x480) && ((t) < dsVIDEO_PIXELRES_MAX))
...
typedef enum _dsVideoFrameRate_t {
  ...
  dsVIDEO_FRAMERATE_59,       ///< Played at 59 frames per second
  dsVIDEO_FRAMERATE_23,       ///< Played at 23 frames per second
...
#define dsVideoPortFrameRate_isValid(t)                                        \
  (((t) >= dsVIDEO_FRAMERATE_UNKNOWN) && ((t) < dsVIDEO_FRAMERATE_MAX))
 ...
 */
#define dsVideoPortScanMode_isValid(t)                                         \
  (((t) >= dsVIDEO_SCANMODE_INTERLACED) && ((t) < dsVIDEO_SCANMODE_MAX))
...
/**
 * @brief This enumeration defines all of the standard video Stereo Scopic
 * modes.
...
#define dsVideoPortStereoScopicMode_isValid(t)                                 \
  (((t) >= dsVIDEO_SSMODE_UNKNOWN) && ((t) < dsVIDEO_SSMODE_MAX))
...
#define dsVideoPortAspectRatio_isValid(t)                                      \
  (((t) >= dsVIDEO_ASPECT_RATIO_4x3) && ((t) < dsVIDEO_ASPECT_RATIO_MAX))

/**
 * @brief Structure that defines video port resolution settings of output video
 * device.
...
typedef struct _dsVideoPortResolution_t {
  ...
  dsVideoResolution_t
      pixelResolution;              ///< The resolution associated with the name
  dsVideoAspectRatio_t aspectRatio; ///< The associated aspect ratio
  dsVideoStereoScopicMode_t
      stereoScopicMode;         ///< The associated stereoscopic mode
  dsVideoFrameRate_t frameRate; ///< The associated frame rate
  bool interlaced; ///< The associated scan mode( @a true if interlaced, @a
                   ///< false if progressive )
...
typedef struct _dsVideoPortTypeConfig_t {
  ...
  dsVideoPortResolution_t
      *supportedResolutions; ///< List of supported resolutions
...
typedef enum _dsHdcpStatus_t {
  ...
  dsHDCP_STATUS_UNAUTHENTICATED, ///< HDCP Authentication Process is not
                                 ///< initiated
  dsHDCP_STATUS_AUTHENTICATED, ///< HDCP Authentication Process is initiated and
                               ///< Passed
  dsHDCP_STATUS_AUTHENTICATIONFAILURE, ///< HDCP Authentication Failure or Link
                                       ///< Integroty Failure
 ...
 * @see dsHdcpStatus_t
 */
#define dsHdcpStatus_isValid(t)                                                \
  (((t) >= dsHDCP_STATUS_UNPOWERED) && ((t) < dsHDCP_STATUS_MAX))
...
typedef enum dsAviContentType {
  ...
  dsAVICONTENT_TYPE_NOT_SIGNALLED, /*!< Content type no data   - ITC=0, CN1=0,
                                      CN0=0 */
  dsAVICONTENT_TYPE_MAX,           /*!< Out of range */
} dsAviContentType_t;

/**
 * @brief This enumeration defines all of the AVI InfoFrame scan information
 * types.
 */
...
#define dsEEDID_MAX_MON_NAME_LENGTH                                            \
  14 ///< Maximum Length of Connected Display Monitor Name
...
typedef struct _dsDisplayEDID_t {
  ...
  bool isRepeater; ///<  Receiver is a repeater  ( @a true if Repeater, @a false
                   ///<  if connected Receiver is not a repeater)
  ...
  dsVideoPortResolution_t
      suppResolutionList[dsEEDID_MAX_VIDEO_CODE *
                         dsVIDEO_SSMODE_MAX];    ///<  EDID Supported Resoultion
                                                 ///<  List
  char monitorName[dsEEDID_MAX_MON_NAME_LENGTH]; ///<  Connected Display Monitor
                                                 ///<  Name
} dsDisplayEDID_t;
...
typedef enum _dsDisplayMatrixCoefficients_t {
  ...
  dsDISPLAY_MATRIXCOEFFICIENT_BT_470_2_BG, ///< ITU BT 470_2_BG Matrix
                                           ///< Coefficient
  ...
  dsDISPLAY_MATRIXCOEFFICIENT_BT_2020_NCL, ///< ITU BT 2020 non constant
                                           ///< luminance Matrix Coefficient
  dsDISPLAY_MATRIXCOEFFICIENT_BT_2020_CL,  ///< ITU BT 2020 constant luminance
                                           ///< Matrix Coefficient
  dsDISPLAY_MATRIXCOEFFICIENT_eDVI_FR_RGB, ///< eDVI Full Range RGB Matrix
                                           ///< Coefficient
  ...
  dsDISPLAY_MATRIXCOEFFICIENT_eHDMI_FR_YCbCr, ///< eHDMI Full Range YcbCr Matrix
                                              ///< Coefficient
...
typedef enum _dsAudioInput_t {
  dsAUDIO_INPUT_PRIMARY =
      0, ///< Primary Audio Input is main audio input provided to audio mixer
